...
// Configuration defaults and browser-mode persistence live in lib/localConfig
import {
  DEFAULT_CONFIG,
  loadLocalConfig,
  saveLocalConfig,
...
interface SettingsModalProps {
  ...
  /** True when the app runs without a backend (static deployment). */
  browserMode?: boolean;
}
...
export const SettingsModal: React.FC<SettingsModalProps> = ({
  ...
  // Load current configuration on open: from the browser store when there is
  // no backend, otherwise from the server API.
  useEffect(() => {
    if (!isOpen) return;
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    // Browser engine: persist locally instead of calling the server API
    if (browserMode) {
    ...
    >
      ...
      >
        ...
        <form onSubmit={handleSave} className="flex-1 overflow-y-auto p-6 space-y-6">
          {browserMode && (
            <div className="p-3 bg-[#ff8a00]/10 border border-[#ff8a00]/30 rounded-xl text-[11px] text-[#ff8a00] flex items-start gap-2">
              <Info className="w-4 h-4 shrink-0 mt-0.5" />
              <span>
                Browser engine active: limits are saved in this browser and applied to
                in-browser scans. Server-only settings (rate limit, CIDR cache) don&apos;t
                apply.
              </span>
            </div>
          )}
          ...
          {saveSuccess && (
            <div className="p-3 bg-emerald-500/10 border border-emerald-500/30 rounded-xl text-emerald-400 text-xs flex items-center gap-2">
              ...
              <span>
                {browserMode
                  ? "Saved in this browser — the browser scan engine will use these limits."
                  : "Variables successfully saved and applied to active server & .env!"}
              </span>
            </div>
          )}
