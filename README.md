...
| **Cloudflare Workers** | [![Deploy to Cloudflare](https://deploy.workers.cloudflare.com/button)](https://deploy.workers.cloudflare.com/?url=https://github.com/Jeeva-zone/google-ai-scan-domain) | Uses `wrangler.jsonc` (assets-only Worker serving `dist/`) and `public/_headers`. Static hosting — scans run in the browser. |
...
> The button clones the repository's **default branch**, which carries the same `wrangler.jsonc`.
...
- **`GET /api/*`** is answered by the SPA fallback with `index.html`, so the app detects that no
  backend exists and switches to the **browser scan engine** (crt.sh + DNS-over-HTTPS + the bundled
  Cloudflare CIDR snapshot) — see [Scan Engines](#-scan-engines). Certificate Transparency discovery
  and DNS resolution then happen in the visitor's browser.
...
> Want the server-side engine (live CIDR fetching, Python concurrency, rate limiting) instead?
> Deploy to **Vercel** or **Netlify**, which run the Python handlers under `/api`.
